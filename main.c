...
unsigned int GetPageType(unsigned int p) //获取page的类型
{
...
void GetWordLineNumber(unsigned int p,unsigned int *number,unsigned int *type) //根据page，获取page所属的wordline，以及page的类型，并存储。
{
    *type = GetPageType(p);
    if(*type == UPPER_PAGE)
        p = GET_LOWER_PAGE(p);
...
void DistributionRowColumn(unsigned int number,unsigned int type,unsigned int *row ,unsigned int *column) //// 对应到相应的二维数组上，即数据的一个元素代表一个wordline里，某种类型的所有page的总的信息，如：sum，max，count。
{
    if(type == UPPER_PAGE)
    {
        *row = 1;
...
void Select_Data(unsigned int row,unsigned int column,short value)      // 获取每个节点内部的信息
{
...
unsigned int c2h(unsigned char c)                                           //字符装换为数字
{
    if((c >= '0')
            && (c <= '9'))
        return c - '0';
...
unsigned int do_int(char *fp,int length)    //字符串转换为16进制数
{
...
void different_wordline(struct dirent * fileinfo)  // 拿出目录中的rdber文件信息流
{
    FILE *fp = NULL;
    int file_length = 0;
    ...
    while(file_length > 0)
    {
        ...
        Select_Data(row, column , (first_value + second_value));           //  存储数组元素信息
...
{                                                                               //1.rdber文件所在的目录 ，如：\fifth_test\normal_tempurature_7\2_pattern\5055_4_0_0_0\all_nand
    ...
    while(endurance_number <= 4)                                                    // 根据Endurance先将block分类，然后
    {
       ...
       // printf("count_number:%d\n",count_number);
        ...
        write_into_file(path,endurance_number,dest_dir_name,big_dir);    // 将Endurance相同的block，分析得到数据存储在同一个文件里
...
int main(int argc,char *argv[])
{
    ...
    while((info_bake = readdir(dir_bake)) != NULL)
    {
       ...
       bake_name_length = strlen (info_bake->d_name);
        strncpy(search_dir_name,info_bake->d_name + 11,bake_name_length - 11);//取info_bake里面名字的从第11个字符开始的两个字符
        //printf("----------------%s\n",search_dir_name);
        if(info_bake->d_type != DT_DIR)
            continue;

        strcat(bake_path,"/");
        strcat(bake_path,info_bake->d_name);    // 追加目录
        ...
        while((info_offset = readdir(dir_offset)) != NULL)
        {
            ...
            memset(offset_path,0,sizeof(offset_path));//每一次运行进行一次至零。
